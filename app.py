...
def ask_question():
    try:
        ...
        if chat_history == []:
            ...
            # chat_history = conv_chat_histories[conv_id]
            message, chat_history, topic, tokens_out, tokens_in, tokens_embbed, header_ref_array = ask_with_memory(
                question, source, chat_history, topic)
        print(f"done ask_with_memory")
        print(header_ref_array)
        ...
        for msg in chat_history:
            if msg.type == "human":
                ...
                serialized_msg = {"type": "ai", "content": msg.content, 'header_ref': header_ref_array[i]}
                i=i+1
            else:
                # Handle other message types if needed
                serialized_msg = {"type": "unknown", "content": str(msg)}

            serialized_chat_history.append(serialized_msg)
            # if serialized_chat_history and serialized_chat_history[-1].get('type') == 'ai':
            #     serialized_chat_history[-1]['header_ref'] = header_ref
...
def generate_quiz():
    try:
        ...
        # processQuiz = threading.Thread(target=quiz_builder, args=(quiz_id, number_of_quiz, source, type_of_quiz,
        #                                                           multiple_option_amount, document_id, lang))
        # processQuiz.start()

        quiz, tokens_in, tokens_out, tokens_embbed = quiz_builder(quiz_id, number_of_quiz, source, type_of_quiz,
                                                                  multiple_option_amount, document_id, lang)
